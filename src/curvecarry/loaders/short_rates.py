...
source``) — every series as fetched; and ``data/interim/funding.parquet``
(``date, country, currency, rate, kind, source``) — one row per country,
month and kind, with the EUR splice applied. The funding rate is never placed
on a curve (``decisions/short_anchor.md``).
"""
...
KIND_POLICY, KIND_INTERBANK = "policy", "interbank_3m"
...
def fetch(cfg: dict) -> list[Path]:
    ...
    for area in BIS_AREAS:
        ...
        out.append(path)
    for series in INTERBANK.values():
...
def parse(paths: list[Path]) -> pd.DataFrame:
    ...
    for p in paths:
        ...
        if name.startswith("CBPOL_"):
            ...
            area = name[len("IR3TIB01") : len("IR3TIB01") + 2]
            d = parse_fred_monthly(p, area)
            d["kind"], d["source"] = KIND_INTERBANK, "fred"
        else:
            raise ValueError(f"{p}: not a policy-rate or interbank file")
        frames.append(d)
...
def build_funding(short_rates: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    for country in countries:
        ...
        ib_area = INTERBANK[currency][len("IR3TIB01") : len("IR3TIB01") + 2]
...
def latest_paths() -> list[Path]:
    return [manifest.latest_raw("bis", f"CBPOL_{a}") for a in BIS_AREAS] + [
        manifest.latest_raw("fred", s) for s in INTERBANK.values()
    ]
